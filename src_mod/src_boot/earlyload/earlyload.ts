(async () => {
    ...
    if (!modName) {
        console.error('[SimpleCryptWrapperModPack] earlyload cannot get NowRunningModName');
        logger.error('[SimpleCryptWrapperModPack] earlyload cannot get NowRunningModName');
        return;
    }
    const ref = getRef(modName);
    if (!ref) {
        console.error('[SimpleCryptWrapperModPack] earlyload cannot get ref', [modName]);
        logger.error(`[SimpleCryptWrapperModPack] earlyload cannot get ref [${modName}]`);
        return;
    }
    await ref.decrypt();
})();
