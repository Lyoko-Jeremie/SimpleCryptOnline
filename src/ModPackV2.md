...
* `xxhash64` (`uint64`, 8 bytes): 文件未加密原文的 `xxHash64`，以便验证文件完整性。

* `padding`: 自动填充 `0x00`，确保下一个结构（File Data）绝对从新的 64 字节边界开始。

    * *计算公式: `(64 - ((20 + name_length) % 64)) % 64`*
...
使用 XChaCha20 进行原地加密。只加密文件体所在的块，不加密任何索引或元数据区块。加密时，`Local Header` 中的 `flags` 位标明该文件被加密。

全局文件头中的 `Xchacha20Nonce` 和 `PwhashSalt` 直接存储原始字节，供加密算法使用。加密时，必须确保每个文件数据块的起始 Counter 等于其绝对块编号，以实现真正的零拷贝原地加解密。
